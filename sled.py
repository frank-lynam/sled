...
print("To pick which mods are enabled, put them in the enabled folder, the load order is alphabetical")
...
def recurse_lower(source, dest, lower=True):
  if lower:
    ...
    os.remove(".sledding")
    [os.system(f"mkdir -p {sn(dest + '/' + '/'.join(x[len(source):].lower().split('/')[:-1]))} && cp {sn(x)} {sn(dest + x[len(source):].lower())}") for x in sources if os.path.isfile(x)]
  else: 
...
def check_conflicts(source, dest):
  ...
  with open(".sledding") as fl:
    sources = [x.strip() for x in fl.readlines()]
  os.remove(".sledding")
  dests = [dest + "/" + '/'.join(x[len(source):].lower().split('/')[:-1]) for x in sources if os.path.isfile(x)]
  ...
  if data_path == None and esm_path == None:
    ...
    if data_path != None:
      ...
      if esm_path != None and data_path in esm_path:
        ...
        esm_path = None
      warnings = warnings + [f"File conflict when adding {plugin}: " + x for x in check_conflicts(data_path, "staging/Data")]
...
for x in ["textures", "meshes", "geometries", "sound"]:
  if x.title() in os.listdir("staging/Data") and x in os.listdir("staging/Data"):
